"""
csv_npi_to_duckdb.py

Reads NPI Parquet from RustFS and loads into DuckDB raw schema.
...
Usage:
    uv run python -m src.ingestion.csv_npi_to_duckdb
"""
...
# table names
FINAL_TABLE = settings.npi_final_table
STAGE_TABLE = settings.npi_stage_table

# sql file path
SQL_PATH = os.path.join(os.path.dirname(__file__), "..", "sql", "ingest_npi.sql")
...
def main():
    ...
    s3_key = get_latest_parquet_key(client, "npi/")
    ...
    con.close()
    logger.success("NPI ingestion into DuckDB complete!")
